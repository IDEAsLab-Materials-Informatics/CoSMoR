...
def create_features(df_alloys):
    ...
    from helper_functions import get_comp_avg, get_asymmetry, get_miedema_enthalpy
    
    alloy_name_list = df_alloys["alloy_name"]
    df_feats = pd.DataFrame()
    
    # create asymmetry features
    df_feats["R_delta"] = get_asymmetry(alloy_name_list, feat_key="r")
    df_feats["VEC"] = get_comp_avg(alloy_name_list, feat_key="VEC")
    df_feats["R_cov_delta"] = get_asymmetry(alloy_name_list, feat_key="r_cov")
    df_feats["E_delta"] = get_asymmetry(alloy_name_list, feat_key="E_GPa")
    df_feats["E_coh_avg"] = get_comp_avg(alloy_name_list, feat_key="Coh_E")
    df_feats["H_ch_M_L_R"] = [get_miedema_enthalpy(alloy)["H_chem"] for alloy in alloy_name_list]
    df_feats["H_el_M_S_R"] = [get_miedema_enthalpy(alloy)["H_el"] for alloy in alloy_name_list]
    
    xmin = pd.read_csv(f"helper_data{SEP}xmin_db_features_norm.csv")
    xmax = pd.read_csv(f"helper_data{SEP}xmax_db_features_norm.csv")
...
def make_predictions(df_x_feats):
    ...
    Y_total = np.zeros(shape=(len(df_x_feats), 1))
    for mod in sorted(model_list):
        
        model = tf.keras.models.load_model(model_dir + mod)  #loading the tained model into 'model' variable
        pred_phases = model.predict(df_x_feats)

        Y_current_mod = [pred_phases[i][0] for i in range(0, len(df_x_feats))]
        Y_current_mod = np.array(Y_current_mod).reshape((len(df_x_feats), 1))
        Y_total += Y_current_mod
